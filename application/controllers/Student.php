...
class Student extends CI_Controller
{
    ...
    function student_syllabus($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');

        $student_profile         = $this->db->get_where('std_syllabus', array(
            'id' => $this->session->userdata('id')
        ))->row();
        $page_data['class_id']   = $student_profile->class_id;
        $page_data['page_name']  = 'student_syllabus';
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'change_password') {
            ...
            $current_password = $this->db->get_where('student_pundro', array(
                'id' => $this->session->userdata('id')
            ))->row()->password;
            if ($current_password == $data['password'] && $data['new_password'] == $data['confirm_new_password']) {
                $this->db->where('id', $this->session->userdata('id'));
                $this->db->update('student_pundro', array(
            ...
            //exit();
            redirect(base_url() . 'index.php?student/manage_profile_new/', 'refresh');
        }
    ...
    function class_routine_pundro($param1 = '', $param2 = '')
    {
        ...
        //echo $this->session->userdata('id');
        $this->db->where('id', $this->session->userdata('id'));
        $ttee = $this->db->get('student_pundro')->result_array();
        foreach($ttee as $rrr):
            //echo $rrr['NameofBatch'];
        endforeach;
        //exit();
        $this->db->where('BatchName',$rrr['NameofBatch']);
        $page_data['acdSession']    = $this->db->get('class_routine_pundro')->result_array();
        $page_data['page_name']  = 'class_routine_pundro';
        $page_data['page_title'] = get_phrase('class_routine');
        $this->load->view('backend/index', $page_data);
    }
    function student_dues($param1 = '', $param2 = '', $param3 = ''){
        $page_data['page_name']  = 'student_dues';

        $page_data['page_title'] = get_phrase('student_dues');
        $page_data['edit_data']  = $this->db->get_where('student_pundro', array(
            'id' => $this->session->userdata('id')
        ))->result_array();
        $this->load->view('backend/index', $page_data);
    }
    function student_courses($param1 = '', $param2 = '', $param3 = ''){
        
        
        $page_data['edit_data']    = $this->db->get('assign_subjects')->result_array();
        $page_data['page_name']  = 'student_courses';
        $page_data['page_title'] = get_phrase('student_courses');
        //$page_data['edit_data']  = $this->db->get_where('assign_subjects', array('id' => $this->session->userdata('id')
        //))->result_array();
        $this->load->view('backend/index', $page_data);
    }
    function semester_details($param1 = '', $param2 = '', $param3 = ''){
        $page_data['page_name']  = 'semester_details';

        $page_data['page_title'] = get_phrase('semester_details');
        //$this->session->userdata('id');
        $this->db->where('id', $this->session->userdata('id'));
        $std222 = $this->db->get('student_pundro')->result_array();
        foreach($std222 as $saad):
            //echo $saad['Session'];
        endforeach;
        //exit();
        $this->db->where('Year',$saad['AdmissionYear']);
        $this->db->where('id',$saad['Session']);
        $page_data['sd']  = $this->db->get('session_pundro')->result_array();
        $this->load->view('backend/index', $page_data);
    }
    function manage_profile_new($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('student_login') != 1)
            redirect(base_url() . 'index.php?login', 'refresh');
        if ($param1 == 'update_profile_info') {
            $data['NameofApplicant']        = $this->input->post('name');
            $data['email']       = $this->input->post('email');

            $this->db->where('id', $this->session->userdata('id'));
            $this->db->update('student_pundro', $data);
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/student_image/' . $this->session->userdata('student_id') . '.jpg');
            $this->session->set_flashdata('flash_message', get_phrase('account_updated'));
            redirect(base_url() . 'index.php?student/manage_profile/', 'refresh');
        }
        ...
        $this->session->userdata('id');
        $this->session->userdata('student_login');
        //exit();
        $page_data['page_name']  = 'manage_profile_new';
