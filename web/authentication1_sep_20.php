...
if(isset($_POST['btn-upload']))
{
	...
	$file = rand(1000,100000)."-".$_FILES['photoApplicant']['name'];
	$file_loc = $_FILES['photoApplicant']['tmp_name'];
	$file_size = $_FILES['photoApplicant']['size'];
	$file_type = $_FILES['photoApplicant']['type'];
	$folder="uploads/";
...
<form method="POST" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	<input type="file" name="photoApplicant" />
...
$CandidateStudent  = 0;
if(isset($_POST["btn-upload12"])) {
	$temp = $_POST[PresentMobile];
	echo $file = rand(1000,100000)."-".$_FILES['photoApplicant']['name'];
	//exit();
				   ...
				   <input type="checkbox" name="terms12" value="check" id="terms12">You agree to our Terms and Policy.
					...
					<div class="form-group test32">
						<div class="col-sm-offset-3 col-sm-5">
							...
							<button type="submit" name="btn-upload12" class="btn btn-info"><?php echo "Admission"; ?></button>
						</div>
					</div>
				</form>
			</div>
